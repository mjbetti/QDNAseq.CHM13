%\VignetteIndexEntry{Introduction to QDNAseq.hg19}
...
\title{Introduction to QDNAseq.hg19}
\author{Daoud Sie}
\maketitle
...
They can be obtained though the QDNAseq.hg19 annotation package. The datasets
are listed as follows

\begin{Schunk}
\begin{Sinput}
> data(package="QDNAseq.hg19")
Data sets in package ‘QDNAseq.hg19’:
...
> bins <- loadBinAnnotation(binSize=15)
> bins
...
By default, QDNAseq ignores sex chromosomes. In order to include them in the
analysis, function \code{applyFilters()} should be run with argument
\code{chromosomes=NA} to include both X and Y, or \code{chromosomes="Y"} to
include X only.

However, this will also affect which chromosomes are used when calculating the
loes correction with \code{estimateCorrection()}. Unless the data set is only
females, this could be undesirable. The solution is to first filter out the sex
chromosomes, run \code{estimateCorrection()}, and finally to reverse the
filtering of sex chromosomes:
...
Running \code{estimateCorrection()} and \code{correctBins()} with a different
set of bins can have one side effect. This is caused by the fact that there can
be bins in the sex chromosomes with a combination of GC content and mappability
that is not found anywhere else in the genome. This will cause those bins to
miss a correction estimate altogether, and these bins will be filtered out from
subsequent steps by \code{correctBins()}. If this happens, it will print out a
message speficying the number of bins affected.
